...
mod tests {
    ...
    fn test_h025_orphan_open_tag() {
        ...
        assert!(!h025_errors.is_empty(), "Should detect orphan open <div> tag. Errors: {:?}", errors);
    }
    ...
    fn test_h025_orphan_closing_td_tag() {
        ...
        let h025_errors: Vec<_> = errors.iter().filter(|e| e.code == "H025").collect();
        assert_eq!(h025_errors.len(), 1, "Should detect orphan </td> tag. Errors: {:?}", errors);
    }
    ...
    fn test_h025_orphan_open_select_tag() {
        ...
        let h025_errors: Vec<_> = errors.iter().filter(|e| e.code == "H025").collect();
        assert!(!h025_errors.is_empty(), "Should detect orphan <select> tag. Errors: {:?}", errors);
    }
}
